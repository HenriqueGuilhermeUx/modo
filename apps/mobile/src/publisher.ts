...
export async function startPublisherConnection(
  ...
  if (!payload.authorizationUrl) throw new MobilePublisherError("O provedor não retornou a tela de autorização.", 502);
  return payload.authorizationUrl;
}

export async function importLinkedInConnection(token: string, brandId: string): Promise<NativeConnection> {
  const payload = await request<{ connection: unknown }>(
    "/api/v2/publisher/connections/linkedin/import",
    token,
    { method: "POST", body: JSON.stringify({ brandId }) },
  );
...
export async function startLinkedInConnection(token: string): Promise<string> {
  const payload = await request<{ authorizationUrl?: string }>(
    "/api/v1/linkedin/connect",
    token,
    { method: "POST", body: JSON.stringify({ authorType: "member" }) },
  );
  if (!payload.authorizationUrl) throw new MobilePublisherError("O LinkedIn não retornou a tela de autorização.", 502);
  return payload.authorizationUrl;
}
