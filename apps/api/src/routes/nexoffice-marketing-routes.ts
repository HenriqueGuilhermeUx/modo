...
export async function registerNexOfficeMarketingRoutes(app:FastifyInstance,options:Options={}){
 ...
 app.post("/api/v1/internal/nexoffice/marketing/v1/content/drafts/:id/approve",async(request,reply)=>{const a=access(request,expectedKey);if(!a.ok)return reply.code(a.status).send({error:a.error});try{const id=z.string().uuid().parse((request.params as any).id),approved=await content.approve(id,contentWorkspaces.organizationId(a.workspaceId));return{request:approved,governance:{workspaceScoped:true,explicitApproval:true,billingMode:"nexoffice_entitlement",modoCreditsCharged:0,publishing:false,externalPublication:false}}}catch(e){return contentFail(reply,e)}});
