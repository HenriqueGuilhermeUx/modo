...
export default function IntelligenceWorkspace() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setSuccess(
        provider === "queue"
          ? "Missão salva na fila interna. A configuração externa será ativada após validarmos os Actors e custos."
          : "Missão iniciada. Você pode acompanhar o status abaixo.",
      );
    } catch (caught) {
  ...
  async function retry(item: IntelligenceMission) {
    ...
    try {
      ...
      setMissions((current) => current.map((entry) => entry.id === updated.id ? updated : entry));
      setSuccess("Missão enviada novamente.");
    } catch (caught) {
  ...
  async function showResults(item: IntelligenceMission) {
    ...
    setError("");
    try {
      const result = await getIntelligenceResults(item.id, 30);
      setPreview(JSON.stringify(result.items, null, 2));
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Não foi possível carregar a prévia.");
    } finally {
      setBusy("");
    }
  }
    ...
    <div className="intelligence-shell">
      ...
      <main>
        <section className="intelligence-hero">
          ...
          <aside><small>PROVEDOR ATUAL</small><strong>{providerLabels[provider]}</strong><p>{provider === "queue" ? "Modo seguro: registra as missões sem consumir coleta externa." : "Execução externa habilitada para os playbooks configurados."}</p></aside>
        </section>
        ...
        <section className="intelligence-layout">
          <form onSubmit={submit}>
            ...
            <div className="intelligence-fields two">
              ...
              <label>Limite inicial<input type="number" min={1} max={5000} value={maxItems} onChange={(event) => setMaxItems(Number(event.target.value))} /></label>
            </div>
            ...
            <div className="intelligence-form-footer"><div><small>STATUS DO PLAYBOOK</small><strong>{configured[playbook] ? "Task configurada" : provider === "queue" ? "Pronto para validação interna" : "Task ainda não configurada"}</strong></div><button className="button button-primary" disabled={saving || !brandId}>{saving ? "Criando missão..." : "Criar missão ↗"}</button></div>
          </form>
            ...
            <div className="intelligence-list">
              {missions.map((item) => (
                <article key={item.id}>
                  ...
                  <footer><button type="button" disabled={busy === item.id} onClick={() => void refresh(item)}>Atualizar</button>{item.status === "failed" && <button type="button" disabled={busy === item.id} onClick={() => void retry(item)}>Reenviar</button>}{item.status === "succeeded" && <button type="button" disabled={busy === item.id} onClick={() => void showResults(item)}>Ver prévia</button>}</footer>
                </article>
              ))}
        ...
        {preview && <section className="intelligence-preview"><div><small>PRÉVIA TÉCNICA</small><h2>Primeiros registros</h2><button type="button" onClick={() => setPreview("")}>Fechar</button></div><pre>{preview}</pre></section>}
      </main>
      <style>{`.intelligence-shell{min-height:100vh;background:#f3f6fb;color:#0d1b3e}.intelligence-shell>header{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:18px max(20px,calc((100vw - 1380px)/2));background:#fff;border-bottom:1px solid #dfe6f1}.intelligence-shell header img{width:112px}.intelligence-shell header>div{display:grid;gap:3px}.intelligence-shell header small,.intelligence-form-title small,.intelligence-list-title small,.intelligence-preview small{font-size:9px;letter-spacing:.13em;font-weight:900;color:#1f5eff}.intelligence-shell header>a:last-child{font-size:12px;font-weight:800;color:#1f5eff}.intelligence-shell>main{width:min(1320px,calc(100% - 40px));margin:0 auto;padding:54px 0 80px}.intelligence-hero{display:grid;grid-template-columns:1.15fr .55fr;gap:35px;align-items:end;margin-bottom:28px}.intelligence-hero>div>span{font-size:10px;letter-spacing:.13em;font-weight:900;color:#1f5eff}.intelligence-hero h1{font:800 clamp(45px,5vw,68px)/1.02 Sora,sans-serif;letter-spacing:-.06em;margin:12px 0}.intelligence-hero h1 strong{color:#1f5eff}.intelligence-hero p{color:#5b657a;line-height:1.65;max-width:850px}.intelligence-hero aside{background:#0d1b3e;color:#fff;border-radius:22px;padding:24px}.intelligence-hero aside small{color:#2ed19a;font-size:9px;letter-spacing:.12em;font-weight:900}.intelligence-hero aside strong{font:800 22px Sora,sans-serif;display:block;margin:8px 0}.intelligence-hero aside p{color:#c2cce0;font-size:12px;margin:0}.intelligence-layout{display:grid;grid-template-columns:1.05fr .95fr;gap:18px;align-items:start}.intelligence-layout>form,.intelligence-missions{background:#fff;border:1px solid #dfe6f1;border-radius:24px;padding:25px}.intelligence-form-title h2,.intelligence-list-title h2,.intelligence-preview h2{font:800 28px Sora,sans-serif;margin:6px 0}.intelligence-form-title p{color:#5b657a;margin:0 0 20px}.intelligence-fields{display:grid;gap:13px;margin-top:13px}.intelligence-fields.two{grid-template-columns:1fr 1fr}.intelligence-fields label{display:grid;gap:7px;font-size:11px;font-weight:900}.intelligence-fields label span{font-weight:500;color:#7a8498}.intelligence-fields input,.intelligence-fields select,.intelligence-fields textarea{width:100%;border:1px solid #d8e1ef;border-radius:12px;padding:12px;background:#fbfcfe;color:#0d1b3e;font:500 13px Inter,sans-serif;box-sizing:border-box}.intelligence-fields textarea{min-height:84px;resize:vertical}.intelligence-form-footer{display:flex;align-items:center;justify-content:space-between;gap:20px;margin-top:20px;padding-top:18px;border-top:1px solid #e6ebf3}.intelligence-form-footer>div{display:grid;gap:4px}.intelligence-form-footer small{font-size:8px;color:#7a8498;font-weight:900}.intelligence-form-footer strong{font-size:12px}.intelligence-list-title{display:flex;justify-content:space-between;align-items:center}.intelligence-list-title>span{background:#eef3ff;color:#1f5eff;border-radius:999px;padding:9px 12px;font-weight:900}.intelligence-list{display:grid;gap:10px;margin-top:18px;max-height:720px;overflow:auto;padding-right:3px}.intelligence-list article{border:1px solid #e1e7f0;border-radius:17px;padding:16px}.intelligence-card-head{display:flex;justify-content:space-between;gap:15px}.intelligence-card-head small{font-size:8px;letter-spacing:.1em;color:#1f5eff;font-weight:900}.intelligence-card-head h3{font:800 17px Sora,sans-serif;margin:5px 0}.intelligence-card-head>span{height:max-content;border-radius:999px;padding:7px 9px;background:#fff3d8;color:#8a5b00;font-size:9px;font-weight:900}.intelligence-card-head>span.status-succeeded{background:#e9fbf4;color:#087655}.intelligence-card-head>span.status-failed{background:#fff0f0;color:#a52626}.intelligence-card-head>span.status-running{background:#eaf0ff;color:#1f5eff}.intelligence-list article>p{color:#5b657a;font-size:12px;line-height:1.5}.intelligence-card-meta{display:flex;justify-content:space-between;gap:12px;font-size:9px;color:#7a8498}.intelligence-message{background:#f4f7fb;border-radius:10px;padding:10px;margin-top:11px;color:#536078;font-size:11px;white-space:pre-wrap}.intelligence-list footer{display:flex;gap:7px;margin-top:11px}.intelligence-list button,.intelligence-preview button{border:0;border-radius:9px;padding:8px 10px;background:#0d1b3e;color:#fff;font-weight:800;font-size:10px;cursor:pointer}.intelligence-list button:first-child{background:#eaf0ff;color:#1f5eff}.intelligence-empty{border:1px dashed #cbd7e7;border-radius:16px;padding:30px;text-align:center}.intelligence-empty p{color:#5b657a;font-size:12px}.intelligence-preview{background:#0d1b3e;color:#fff;border-radius:23px;padding:24px;margin-top:18px}.intelligence-preview>div{display:flex;justify-content:space-between;align-items:center}.intelligence-preview pre{overflow:auto;max-height:500px;background:#07122b;border-radius:14px;padding:16px;color:#dbe5f7;font-size:11px}.intelligence-loading{min-height:100vh;display:grid;place-content:center;justify-items:center;gap:15px;background:#f3f6fb;color:#0d1b3e}.intelligence-loading img{width:130px}@media(max-width:980px){.intelligence-hero,.intelligence-layout{grid-template-columns:1fr}}@media(max-width:640px){.intelligence-fields.two{grid-template-columns:1fr}.intelligence-form-footer{align-items:flex-start;flex-direction:column}.intelligence-form-footer .button{width:100%}.intelligence-shell>header>div{display:none}.intelligence-shell>main{width:min(100% - 24px,1320px)}.intelligence-hero h1{font-size:43px}}`}</style>
    </div>
  );
}
