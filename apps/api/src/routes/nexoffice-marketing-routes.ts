...
export async function registerNexOfficeMarketingRoutes(app:FastifyInstance,options:Options={}){
 ...
 app.get("/api/v1/internal/nexoffice/marketing/v1/health",async(request,reply)=>{const a=access(request,expectedKey);if(!a.ok)return reply.code(a.status).send({status:"error",error:a.error});return{status:"ok",contract:"nexoffice-marketing-v1",workspaceId:a.workspaceId,storage:{demand:demand.storage,media:media.storage,campaigns:campaigns.storage,prospecting:prospector.storage,intelligence:intelligence.storage},capabilities:["demand.projects","demand.landing","demand.funnel","demand.leads","demand.outcomes","ads.plan","media.connections.read","media.connections.prepare","google_ads.oauth","google_ads.account_select","google_ads.metrics.read","marketing.insights","campaigns.draft","campaigns.review","campaigns.ready","prospecting.icp","prospecting.campaigns","prospecting.leads","prospecting.discovery","prospecting.outreach_draft","intelligence.market_radar.read","intelligence.market_radar.collect"],workflow:["draft","review","ready"],googleAds:{oauthConfigured:googleAds.configured,apiVersion:googleAds.apiVersion,metricsReadOnly:true},prospecting:{provider:prospector.provider,discoveryRequiresExplicitApproval:true,externalOutreach:false},marketRadar:{provider:intelligence.mode,configured:intelligence.configuredPlaybooks().market_radar,collectionRequiresExplicitApproval:true,externalCommunication:false},externalCampaignActivation:false,externalProspectingOutreach:false,readyRequirements:["explicit_client_approval","authorized_media_account"]}});
