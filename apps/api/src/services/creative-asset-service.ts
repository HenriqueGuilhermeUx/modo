...
export interface StoredCreativeJob {
  ...
  kind: string; objective: string; prompt: string; status: string; approvalStatus?: "pending"|"approved"|"rejected"; assets: CreativeAsset[];
...
export class CreativeAssetService {
  ...
  async initialize() {
    ...
    await this.pool.query(`CREATE TABLE IF NOT EXISTS modo_creative_jobs(
      ...
      status TEXT NOT NULL, approval_status TEXT NOT NULL DEFAULT 'pending', assets JSONB NOT NULL DEFAULT '[]'::jsonb, error TEXT,
  ...
  private map(r:any):StoredCreativeJob{return{id:r.id,organizationId:r.organization_id,brandId:r.brand_id,provider:r.provider,providerJobId:r.provider_job_id,kind:r.kind,objective:r.objective,prompt:r.prompt,status:r.status,approvalStatus:r.approval_status||"pending",assets:r.assets||[],error:r.error,createdAt:new Date(r.created_at).toISOString(),updatedAt:new Date(r.updated_at).toISOString()}}
