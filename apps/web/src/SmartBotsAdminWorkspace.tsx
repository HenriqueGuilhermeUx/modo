...
import {
  listAdminSmartBotsIntakes,
  retryAdminSmartBotsIntake,
  updateAdminSmartBotsStatus,
} from "./smartbots-api";
...
export default function SmartBotsAdminWorkspace() {
  ...
  async function retryDispatch(item: SmartBotsIntake) {
    setBusy(item.id);
    setError("");
    setSuccess("");
    try {
      const updated = await retryAdminSmartBotsIntake(item.id);
      await load();
      setSuccess(
        updated.status === "sent"
          ? `Briefing de ${item.businessName} reenviado com sucesso.`
          : `Reenvio de ${item.businessName} concluído com status ${statusLabels[updated.status]}.`,
      );
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Não foi possível reenviar o briefing.");
    } finally {
      setBusy("");
    }
  }
        ...
        <section className="smartbots-admin-list">
          {items.map((item) => (
            <article key={item.id}>
              ...
              <footer><small>Atualizado em {new Date(item.updatedAt).toLocaleString("pt-BR")}</small><div><button onClick={() => void copyBriefing(item)}>Copiar briefing</button>{item.status === "failed" && <button disabled={busy === item.id} onClick={() => void retryDispatch(item)}>{busy === item.id ? "Reenviando..." : "Reenviar"}</button>}<button disabled={busy === item.id} onClick={() => void changeStatus(item)}>{busy === item.id ? "Atualizando..." : "Atualizar status"}</button></div></footer>
            </article>
          ))}
