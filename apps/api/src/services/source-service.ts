...
async function readLimitedText(response: Response) {
  ...
  if (declared > MAX_BYTES) throw new Error("A página é grande demais para o Quick Start.");
  ...
  try {
    while (true) {
      ...
      if (size > MAX_BYTES) throw new Error("A página é grande demais para o Quick Start.");
...
async function fetchSource(rawUrl: string, redirects = 0): Promise<{ response: Response; url: URL }> {
  ...
  const response = await fetch(url, {
    redirect: "manual",
    headers: {
      "user-agent": "MODO-QuickStart/1.0",
...
export async function extractPublicSource(rawUrl: string): Promise<SourceExtractResponse> {
  ...
  if (!contentType.includes("text/html") && !contentType.includes("text/plain") && !contentType.includes("application/xhtml+xml")) {
    throw new Error("Neste momento, o Quick Start lê artigos, blogs, notícias e páginas de texto.");
  }
