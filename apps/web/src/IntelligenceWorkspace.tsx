...
export default function IntelligenceWorkspace() {
  ...
  async function retry(item: IntelligenceMission) {
    ...
    try {
      ...
      setMissions((current) => current.map((entry) => entry.id === updated.id ? updated : entry));
      await refreshEngine();
      setSuccess("Missão reenviada. O reenvio conta como uma nova pesquisa da franquia.");
    } catch (caught) {
            ...
            <div className="intelligence-list">
              {missions.map((item) => (
                <article key={item.id}>
                  ...
                  <footer><button type="button" disabled={busy === item.id} onClick={() => void refresh(item)}>Atualizar</button>{item.status === "failed" && <button type="button" disabled={busy === item.id || quotaExhausted} onClick={() => void retry(item)}>Reenviar</button>}{item.status === "succeeded" && <button type="button" disabled={busy === item.id} onClick={() => void showResults(item)}>Abrir lista</button>}</footer>
                </article>
              ))}
