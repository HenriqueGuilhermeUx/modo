...
type AccessMode = "loading" | "trial" | "full";
...
export default function IntelligenceCommercialResults({ mission, items, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    getIntelligencePlaybooks()
      .then((engine) => {
        if (active) setAccessMode(engine.quota.plan === "trial" ? "trial" : "full");
      })
      .catch(() => {
        if (active) setAccessMode("trial");
      });
    return () => { active = false; };
  }, []);

  const trial = accessMode === "trial";
  ...
  if (accessMode === "loading") {
    return (
      <section className="commercial-results intelligence-access-loading">
        <div className="portal-spinner" />
        <strong>Preparando seus resultados...</strong>
        <p>Confirmando a franquia de Intelligence antes de exibir a missão.</p>
      </section>
    );
  }
