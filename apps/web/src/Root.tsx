...
function RouteLoading() {
  return (
    <main className="portal-loading" aria-live="polite" aria-busy="true">
...
export default function Root() {
  ...
  if (["/politica-de-privacidade", "/privacy", "/privacy-policy"].includes(path)) {
    return suspended(<PrivacyPolicyPage />);
  }
  if (["/exclusao-de-dados", "/data-deletion"].includes(path)) {
    return suspended(<DataDeletionPage />);
  }
  if (path === "/smartbots.html" || path === "/smartbots") {
    return suspended(<SmartBotsPage />);
  }
  if (path === "/onboarding-smartbots.html" || path === "/app/smartbots") {
    return suspended(<SmartBotsOnboarding />);
  }
  if (path === "/rede-modo/convite") {
    return suspended(<SpecialistApplicationPage />);
  }
  if (path === "/especialistas" || path === "/rede-modo") {
    window.location.replace("/");
    return null;
  }
  if (path === "/admin/smartbots") {
    return suspended(<SmartBotsAdminWorkspace />);
  }
  if (path === "/admin/rede") {
    return suspended(<HumanOperationsAdminWorkspace />);
  }
